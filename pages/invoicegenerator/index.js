...
export default function GlobalInvoiceGenerator() {
    ...
    const [meta, setMeta] = useState({
        ...
        senderAddress: 'Office 101, Dubai, UAE',
    ...
    const [items, setItems] = useState([{ id: 1, code: 'S01', desc: 'Item Description', qty: 1, price: 100, vatRate: 5 }]);

    useEffect(() => {
        const saved = localStorage.getItem('shb_invoice_final_v10');
        if (saved) {
    ...
    const saveSettings = () => {
        localStorage.setItem('shb_invoice_final_v10', JSON.stringify({ meta, items }));
        showToast('Business Template Saved Locally! ✅');
    };

    // --- ACCURATE AMOUNT IN WORDS ENGINE ---
    const toWords = (total) => {
        const amount = parseFloat(total).toFixed(2);
        const [dirhams, fils] = amount.split('.');
        ...
        const dText = parseInt(dirhams) > 0 ? convert(dirhams) + ' ' + meta.currency : '';
        const fText = parseInt(fils) > 0 ? (dText ? ' and ' : '') + convert(fils) + ' Fils' : '';
        return (dText + fText + ' Only').trim();
    };
    ...
    const exportFile = async (type) => {
        ...
        showToast('Optimizing high-resolution document...');

        const canvas = await toCanvas(invoiceRef.current, {
            pixelRatio: 3, // Ultra Sharp
            backgroundColor: '#ffffff',
            canvasWidth: 800 // Forced width to prevent clipping
        });
        ...
        let heightLeft = imgH;
        let position = 0;
        const pageHeaderPadding = 25; // Professional gap at top of Page 2+
        ...
        while (heightLeft >= 0) {
            ...
            position = heightLeft - imgH + pageHeaderPadding;
            pdf.addImage(imgData, 'PNG', 0, position, pWidth, imgH);
            heightLeft -= (pHeight - pageHeaderPadding);
        }
        
        if (type === 'png') {
            const link = document.createElement('a');
            link.download = `Invoice-${meta.invoiceNum}.png`; link.href = imgData; link.click();
        } else {
    ...
    return (
        <ToolboxLayout title="Universal Tax Invoice Suite" description="UAE FTA Compliant. Adjustable Color, Multi-page PDF, Amount in words.">
                ...
                <div style={{ display: 'flex', gap: '25px', flexWrap: 'wrap' }}>
                    ...
                    <aside style={sidebarS}>
                        <div style={sidebarCard}>
                            ...
                            <h3 style={{...cardTitle, marginTop:'20px'}}>Currency & Page</h3>
                        ...
                        <div ref={invoiceRef} id="invoice-paper" style={{ 
                            ...
                            <div style={headerGrid}>
                                ...
                                <div style={{ textAlign: 'right' }}>
                                    ...
                                    <div style={mRow}><span>Date of Supply:</span><input type="date" value={meta.supplyDate} onChange={(e)=>setMeta({...meta, supplyDate: e.target.value})} style={mInp} /></div>
                            ...
                            <button onClick={()=>setItems([...items, {id:Date.now(), code:'', desc:'', qty:1, price:0, vatRate:5}])} style={addB} className="no-print">+ Add Item Row</button>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '30px', pageBreakInside: 'avoid' }}>
                                ...
                                <div style={{ width: '240px' }}>
                                    ...
                                    <div style={sumR}><span>VAT Total</span><span>{fmt(totalVat)}</span></div>
                                    <div style={{...grandRow, borderLeft: `4px solid ${meta.titleColor}`, color: meta.titleColor}}>
                            ...
                            <div style={pFoot}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px' }}>
                                    ...
                                    <span style={{ fontSize: '0.65rem', color: '#94a3b8' }}>Professional Document generated by shbstores.com</span>
                                </div>
                            </div>
                        </div>
                    </main>
...
const paperStyle = { background: '#fff', padding: '20mm 15mm', display: 'flex', flexDirection: 'column', minHeight: '297mm', boxShadow: '0 10px 50px rgba(0,0,0,0.5)' };
...
const notesArea = { width: '100%', height: '60px', background: '#f8fafc', padding: '8px', fontSize: '0.75rem', borderRadius:'5px' };
...
const delB = { border: 'none', background: 'none', color: '#f87171', cursor: 'pointer', fontSize: '1.2rem' };