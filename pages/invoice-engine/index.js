...
// 1️⃣ ENTERPRISE JURISDICTIONS CONFIG
const JURISDICTIONS = {
...
const numberToWords = (total, config) => {
    ...
    const precision = config?.decimals || 2;
...
export default function MasterInvoiceEngine() {
    ...
    // --- 4️⃣ RE-CALCULATION & RECOVERY ---
    useEffect(() => {
        setMounted(true);
        const saved = localStorage.getItem('shb_v16_enterprise_engine');
        if (saved) {
            try {
            ...
            <div style={{ maxWidth: '1650px', margin: '0 auto', padding: '40px 20px' }}>
                ...
                <div style={{ display: 'flex', gap: '30px', flexWrap: 'wrap' }}>
                    ...
                    <aside style={sidebarArea}>
                        <div style={panelCard}>
                            <h3 style={hS}>1. System Architecture</h3>
                            ...
                            <h3 style={hS}>2. Advanced Columns</h3>
                            <div style={colList}>
                                {cols.map((c, i) => (
                            ...
                            <button onClick={()=>setCharges([...charges, {id:Date.now(), label:'Service', value:0, type:'fixed', taxable:false}])} style={addB}>+ Add Extra Charge</button>

                            <h3 style={hS}>4. Dynamic Branding</h3>
                            <label style={lC}>Logo & Signature</label>
                            ...
                            <label style={lC}>Signature Align</label>
                            <select value={meta.sigAlign} onChange={(e)=>setMeta({...meta, sigAlign: e.target.value})} style={selS}>
                                <option value="flex-start">Left</option><option value="center">Center</option><option value="flex-end">Right</option>
                            </select>

                            <button onClick={()=>{
                                localStorage.setItem('shb_v16_enterprise_engine', JSON.stringify({ meta, cols, items, charges }));
                                showToast('Settings Saved! ✅');
                            }} style={btnSave}>💾 PERSIST ENGINE DATA</button>
                        ...
                        <div ref={invoiceRef} id="invoice-paper" style={{...paperS, color: meta.pColor}}>
                            ...
                            <div style={headerG}>
                                ...
                                <div style={{flex:1, textAlign:'right'}}>
                                    <div style={mRow}><span>No:</span><input value={meta.iNum} onChange={(e)=>setMeta({...meta, iNum:e.target.value})} style={mInp} /></div>
                                ...
                                <tbody>
                                    {filteredItems.map(row => (
                                        <tr key={row.id} style={{borderBottom:'1px solid #f1f5f9'}}>
                                            ...
                                            <td style={{...tdS, textAlign:'right', fontWeight:'bold'}}>{fmt(row.grand)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                            <button onClick={()=>setItems([...items, {id:Date.now(), values:{qty:1, rate:0, tax:5}}])} style={addB} className="no-print">+ ADD NEW ROW</button>
                            <div style={{display:'flex', justifyContent:'space-between', mt:'50px', breakInside:'avoid'}}>
                                <div style={{width:'55%'}}>
                                    <p style={lC}>TOTAL IN WORDS</p><p style={{fontWeight:'bold', color: meta.pColor, mb:'20px'}}>{numberToWords(engine.grandTotal, engine.config)}</p>
                                ...
                                <div style={{width:'240px'}}>
                                    ...
                                    <div style={{...sumR, borderTop:`2px solid ${meta.pColor}`, fontWeight:'900', fontSize:'1.2rem', padding:'10px 0'}}>
...
const btnSave = { width: '100%', background: '#34d399', color: '#0f172a', padding: '14px', borderRadius: '10px', fontWeight: 'bold', border: 'none', cursor: 'pointer', mb: '10px' };
const btnMain = { width: '100%', background: '#38bdf8', color: '#0f172a', border: 'none', padding: '18px', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer' };
...
const areaI = { fontSize: '0.8rem', color: '#475569', height: '50px', resize: 'none' };
const headerG = { display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #000', pb: '15px' };
...
const tableS = { width: '100%', borderCollapse: 'collapse', mt: '20px' };
...
const addB = { width: '100%', padding: '10px', background: '#f8fafc', border: '1px dashed #ccc', cursor: 'pointer', mt: '15px', color: '#94a3b8', fontSize: '0.7rem', borderRadius:'8px' };
